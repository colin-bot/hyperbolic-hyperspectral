...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.onebyoneconv = None
        if onebyoneconv:
            self.onebyoneconv = nn.Sequential(
                    nn.Conv2d(204, onebyoneconvdim, kernel_size=1),
                    nn.BatchNorm2d(onebyoneconvdim),
                    nn.ReLU(),
                )
    ...
    def forward(self, x):
        if self.onebyoneconv:
            out_0 = self.onebyoneconv(x)
        else: out_0 = x

        out = self.conv1(out_0)
